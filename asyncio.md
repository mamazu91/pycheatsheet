...
2. **Треды:** кол-во тредов, которое можно запустить на CPU, ограничено в отличие от кол-ва корутин. Для корректного взаимодействия между тредами необходимо добавлять lock-механизмы, которые не так просты в обращении. Так же треды потребляют гораздо больше памяти. Один из основных юзкейсов тредов - продолжение выполнения кода программы в главном треде, в то время как другой тред ожидает завершения IO операций.
